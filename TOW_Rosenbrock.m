...
Lb=-30*ones(1,30); % Lower bounds of design variables.
Ub=30*ones(1,30); % Upper bounds of design variables.
...
for i=1:nT
    [X,fit,pfit]=fobj_Rosenbrock(T(i,:),Lb,Ub);
    T(i,:)=X;
    Fit(i)=fit;
    PFit(i)=pfit;
end
...
while NFEs<maxNFEs
    ...
    for i=1:nT
        [X,fit,pfit]=fobj_Rosenbrock(newT(i,:),Lb,Ub);
        newT(i,:)=X;
        newFit(i)=fit;
        newPFit(i)=pfit;
    end
