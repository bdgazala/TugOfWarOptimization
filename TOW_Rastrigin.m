...
nV=30; % Number of design variables.
Lb=-5.12*ones(1,30); % Lower bounds of design variables.
Ub=5.12*ones(1,30); % Upper bounds of design variables.
...
for i=1:nT
[X,fit,pfit]=fobj_Rastrigin(T(i,:),Lb,Ub);
T(i,:)=X;
Fit(i)=fit;
PFit(i)=pfit;
end
...
for i=1:nT
[X,fit,pfit]=fobj_Rastrigin(newT(i,:),Lb,Ub);
newT(i,:)=X;
newFit(i)=fit;
newPFit(i)=pfit;
end
