...
clear
%% Initialization
load('tsb_data_orig.mat')
len=length(ts1400);
C=zeros(len,len);
for i=1:len
    for j=1:len
        variable= (ts1400(i,2)-ts1400(j,2))^2+(ts1400(i,3)-ts1400(j,3))^2;
        C(i,j)=sqrt(variable);
    end
end

% Define the properties of COP (tension/compression spring design problem).
nV=len; % Number of design variables.
Lb=0*ones(1,len); % Lower bounds of design variables.
Ub=2100*ones(1,len); % Upper bounds of design variables.
...
maxNFEs=20000*len; % Maximum Number of Objective Function Evaluations.
%Generate random initial solutions.
for i=1:nT
    T(i,:)=randperm(len); % Teams matrix or the league or matrix
...
for i=1:nT
    [X,fit,pfit]=fobj_TSM(T(i,:),Lb,Ub,C);
    T(i,:)=X;
    Fit(i)=fit;
    PFit(i)=pfit;
end
...
%algorithm until yet.
NITs=0; % Numaber of algorithm iterations
while NFEs<maxNFEs
    %%plot
    CurrentCityPositionSeq = [];
    for i=1:length(ts1400)
        CurrentCityPositionSeq =[CurrentCityPositionSeq;ts1400(bestT(i),2),ts1400(bestT(i),3)];
    end
    ...
    grid on;
    title(['iteration' num2str(NFEs)], 'FontSize', fontSize);
    ...
    %     diffrent=50;
    maximum_iteration=zeros(nT,2);
    for i=1:nT
        for j=2:length(ts1400)-1
            if maximum_iteration(i,1)<stepsize(i,j)
                maximum_iteration(i,1)=stepsize(i,j);
                maximum_iteration(i,2)=j;
            end
        end
    end
    for i=1:nT
        try
        temp=T(i,maximum_iteration(i,2));
        temp_1=T(i,maximum_iteration(i,2)-1);
        T(i,maximum_iteration(i,2)-1)=temp;
        T(i,maximum_iteration(i,2))=temp_1;
        catch ME
    ...
    %     for i=1:nT
    %         for j=2:length(ts1400)-1
    %             if(stepsize(i,j-1)<stepsize(i,j)-diffrent&&stepsize(i,j)<stepsize(i,j+1)+diffrent)
    %                 temp=T(i,j);
    %                 temp_1=T(i,j-1);
    %                 temp2=T(i,j+1);
    %                 T(i,j-1)=temp;
    ...
    for i=1:nT
        [X,fit,pfit]=fobj_TSM(newT(i,:),Lb,Ub,C);
        disp(['T= ' num2str(i) '; fit = ' num2str(fit) ]);
        newT(i,:)=X;
        newFit(i)=fit;
        newPFit(i)=pfit;
    end
